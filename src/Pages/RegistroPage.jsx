...
export const RegistroPage = () => {
  ...
  const getFieldError = (field) => {
    ...
    switch (field) {
      ...
      case 'password': {
        ...
        if (formData.password.length > 100) return 'Máximo 100 caracteres'
        return null
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await authService.registro(formData.nombre, formData.email, formData.password)
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400:
            if (errorMsg.toLowerCase().includes('email') || errorMsg.toLowerCase().includes('existe') || errorMsg.toLowerCase().includes('registrado')) {
              toast.error('Este email ya está registrado. ¿Querés iniciar sesión?', { duration: 5000 })
              ...
              toast.error(errorMsg || 'Datos inválidos. Verificá la información.')
            }
            break
          ...
          case 422:
            toast.error('Los datos ingresados no son válidos. Revisá el formulario.')
            break

          case 429:
            toast.error('Demasiados intentos de registro. Esperá unos minutos.')
            break
        ...
        toast.error('No se pudo conectar con el servidor. Verificá que el backend esté corriendo.', {
          duration: 5000
        })
      } else {
        ...
        <form className="mt-6 sm:mt-8 space-y-4 sm:space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex items-start">
            ...
            <label htmlFor="terms" className="ml-2 block text-xs sm:text-sm text-gray-700 cursor-pointer">
              Acepto los{' '}
              <a href="/terminos" className="text-[var(--persian-plum-600)] hover:text-[var(--persian-plum-700)] font-medium">
                términos y condiciones
              </a>
              {' '}y la{' '}
              <a href="/privacidad" className="text-[var(--persian-plum-600)] hover:text-[var(--persian-plum-700)] font-medium">
